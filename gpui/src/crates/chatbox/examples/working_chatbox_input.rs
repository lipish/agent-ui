// Working Chatbox with proper text input based on GPUI's official example
// Implements EntityInputHandler for proper text editing functionality
...
// Simplified cursor blink manager
...
impl CursorBlinker {
    ...
    pub fn enable(&mut self, cx: &mut Context<Self>) {
        ...
        self.enabled = true;
        // Start with cursor visible and begin blinking
        self.visible = true;
...
impl EntityInputHandler for ChatInput {
    ...
    fn character_index_for_point(
        ...
        // No assertion needed - placeholder text may differ from content
...
impl WorkingChatbox {
    pub fn new(cx: &mut App) -> Self {
        ...
        Self {
            messages: vec![
                "🎉 Welcome to the Working Chatbox!".to_string(),
                "✨ This input field actually works!".to_string(),
                "💪 Try typing, selecting, copying, and pasting!".to_string(),
                "⌨️ Press Enter to send, Ctrl+K to clear!".to_string(),
            ],
            chat_input,
            focus_handle: cx.focus_handle(),
        }
    ...
    fn send_message(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        let message_text = self.chat_input.read(cx).get_text();
        if !message_text.trim().is_empty() {
            self.messages.push(format!("You: {}", message_text));
    ...
    fn handle_send(&mut self, _: &Send, window: &mut Window, cx: &mut Context<Self>) {
        self.send_message(window, cx);
    }

    fn handle_clear(&mut self, _: &Clear, window: &mut Window, cx: &mut Context<Self>) {
        self.clear_chat(window, cx);
    }
}

impl Render for WorkingChatbox {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            .bg(rgb(0xf8fafc))
            .track_focus(&self.focus_handle(cx))
            .on_action(cx.listener(Self::handle_send))
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child("✅ Working Chatbox - Real Text Input")
                    )
            )
            .child(
                // Messages area
                        ...
                        div()
                                ...
                                div()
                                    ...
                                    .child(
                                        ...
                                        div()
                                            ...
                                            .text_size(px(14.0))
                                            .font_weight(FontWeight::MEDIUM)
                                            .child("Send (Press Enter)")
                                    )
                                    .child(
                                        // Clear button (static for now)
                                        div()
                                            .px_6()
                                            .py_3()
                                            .bg(rgb(0xef4444))
                            ...
                            .child(
                                div()
                                    ...
                                    .text_size(px(12.0))
                                    .child("💡 Tips: The input field supports full text editing - arrow keys, Home/End, Ctrl+A to select all, copy/paste, etc.")
                            )
                    )
            )
            .child(
                                ...
                                div()
                                    ...
                                    .child(
                                        div()
                                            ...
                                            .text_size(px(13.0))
                                            .font_weight(FontWeight::MEDIUM)
                                            .child("✅ Full-Featured Input Working")
                                    )
                            )
                            .child(
                                div()
...
impl Focusable for WorkingChatbox {
    fn focus_handle(&self, _: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}
...
fn main() {
    Application::new().run(|cx: &mut App| {
        ...
        cx.bind_keys([
            ...
            KeyBinding::new("enter", Send, None),
            KeyBinding::new("ctrl-k", Clear, None),
        ]);

        match cx.open_window(
            WindowOptions {
                titlebar: Some(gpui::TitlebarOptions {
                    title: Some("Working Chatbox - Real Text Input".into()),
            ...
            |_window, cx| {
                let chatbox = cx.new(|cx| WorkingChatbox::new(cx));
                chatbox.into()
            },
        ) {
            Ok(_) => println!("✅✅✅ WORKING CHATBOX WITH REAL TEXT INPUT LAUNCHED! ✅✅✅"),
